<template>
    <button
        @click="toggle"
        :disabled="loading"
        class="p-2 rounded-full transition hover:scale-110 disabled:opacity-50"
...
export default {
    ...
    methods: {
        async toggle() {
            ...
            try {
                const response = await window.axios.post(`/maps/${this.mapId}/favorite`);
                this.isFavorite = response.data.is_favorite;
                this.$emit('update', { mapId: this.mapId, isFavorite: this.isFavorite });
            } catch (error) {
